#ifndef WIDGET_H
#define WIDGET_H

#include <QWidget>
#include <QLabel>
#include <QSqlDatabase>
#include <QSqlQuery>
#include <QSqlError>
#include <QDebug>
#include <QSqlQueryModel>
#include <QFileDialog>

#include <QtCharts>
#include <QList>
#include <QSplineSeries>
#include <QScatterSeries>
#include <QChart>
#include <QChartView>

#include "readcsvdata.h"
#include "connectdatabase.h"

//T为1-9台站每个曲线图表的XYZ三轴方向
enum T{
    T1X,T1Y,T1Z,
    T2X,T2Y,T2Z,
    T3X,T3Y,T3Z,
    T4X,T4Y,T4Z,
    T5X,T5Y,T5Z,
    T6X,T6Y,T6Z,
    T7X,T7Y,T7Z,
    T8X,T8Y,T8Z,
    T9X,T9Y,T9Z
};

QT_BEGIN_NAMESPACE
namespace Ui { class Widget; }
QT_END_NAMESPACE

class Widget : public QWidget
{
    Q_OBJECT

public:
    Widget(QWidget *parent = nullptr);
    ~Widget();

    void showTable();                               //在UI上显示数据库列表
    void initCharts();                              //初始化曲线图表
 //   void initTButtonCharts();
    void drawSplineWave(int axis_x, double data);   //绘制曲线图
    bool eventFilter(QObject *obj,QEvent *event);
    void mouseMoveEvent(QMouseEvent *event);
    void mouseReleaseEvent(QMouseEvent *event);
    void charViewEventFilter(QEvent *event,QChart *tempChart);   //chartView事件过滤封装函数
private slots:
    void on_axWidget_ImplementCommandEvent(int iCommandId);      //axWidget控件命令

    void zoomInClicked();             //放大按钮
    void zoomOutClicked();            //缩小按钮
    void restoreButtonClicked();      //还原按钮
    void moveViewButtonClicked();     //移动视区按钮

    void startButtonClicked();        //运行按钮
    void dailyStatementClicked();     //日报表按钮
    void monthlyStatementClicked();   //月报表按钮
    void helpButtonClicked();         //帮助按钮

    void timeOutEvent();              //定时器超时事件

    void displayButtonClick();        //显示波形图按钮
    void stopButtonClick();           //暂停按钮
    void fulllScreenButtonClicked();  //全屏按钮

    void on_T1Button_clicked();       //T1站台
    void on_t1X_clicked();            //T1站台X方向
    void on_t1Y_clicked();            //T1站台Y方向
    void on_t1Z_clicked();            //T1站台Z方向

    void on_T2Button_clicked();
    void on_t2X_clicked();
    void on_t2Y_clicked();
    void on_t2Z_clicked();

    void on_T3Button_clicked();
    void on_t3X_clicked();
    void on_t3Y_clicked();
    void on_t3Z_clicked();

    void on_T4Button_clicked();
    void on_t4X_clicked();
    void on_t4Y_clicked();
    void on_t4Z_clicked();

    void on_T5Button_clicked();
    void on_t5X_clicked();
    void on_t5Y_clicked();
    void on_t5Z_clicked();

    void on_T6Button_clicked();
    void on_t6X_clicked();
    void on_t6Y_clicked();
    void on_t6Z_clicked();

    void on_T7Button_clicked();
    void on_t7X_clicked();
    void on_t7Y_clicked();
    void on_t7Z_clicked();

    void on_T8Button_clicked();
    void on_t8X_clicked();
    void on_t8Y_clicked();
    void on_t8Z_clicked();

    void on_T9Button_clicked();
    void on_t9X_clicked();
    void on_t9Y_clicked();
    void on_t9Z_clicked();

private:
    Ui::Widget *ui;

    ConnectDataBase *db;              //定义一个连接数据库的对象

    ReadCSVData *readData;            //定义一个读取CSV文件数据的对象

    QString filePath;                 //CSV文件路径

    QVector<QPointF> pointBuffer;     //坐标点缓存
    QTimer *drawTimer;                //定时器
    bool isStoppingTimer;             //判断定时器是否开启

    QStringList originList;           //CSV数据列表
    QString origin;
    int originListSize;               //CSV数据长度
    int originListIndex;              //当前CSV数据索引

//    QString **mSenChannel ;         //存储每个事件后三个通道中Z轴的数据
//    QString **mSenChannelNum ;      //存储每个事件触发台站名称
//    QString *mDate;                 //存储每个事件的日期
//    int *mMotiPos;                  //每个传感器的激发位置
//    int mSenNum = 0;                //记录传感器的数量

    QSplineSeries *splineSeries;      //曲线序列
    QScatterSeries *scatterSeries;    //点序列
    QChart *chart;                    //图表
    QValueAxis *axisX;                //X坐标轴
    QValueAxis *axisY;                //Y坐标轴
    QChartView *view;                 //视图

    QChart *chartTButton;
    QChartView *viewTButton;

    QVBoxLayout *chartViewLayout1;
    QWidget *chartViewWidget1;
    QVBoxLayout *chartViewLayout2;
    QWidget *chartViewWidget2;
    QVBoxLayout *chartViewLayout3;
    QWidget *chartViewWidget3;

    bool isClickingChart;                           //chart图表鼠标移动事件的开关
    int xOld;                                       //chart图表鼠标移动的X轴距离
    int yOld;                                       //chart图表鼠标移动的Y轴距离

};
#endif // WIDGET_H
